(() => {
    ...
    const backdrop = modal.querySelector(
        "[data-case-modal-backdrop]"
    );

    const modalTitle = modal.querySelector(
        "[data-case-modal-title]"
    );

    const modalDescription = modal.querySelector(
        "[data-case-modal-description]"
    );

    const editIcon = modal.querySelector(
        "[data-case-modal-icon-edit]"
    );

    const createIcon = modal.querySelector(
        "[data-case-modal-icon-create]"
    );
    ...
    function setCreateHeader() {
        if (modalTitle) {
            modalTitle.textContent = "Novo caso jurídico";
        }

        if (modalDescription) {
            modalDescription.textContent =
                "Cadastre uma nova demanda jurídica vinculada ao cliente.";
        }
    ...
    function setEditHeader() {
        if (modalTitle) {
            modalTitle.textContent = "Editar causa";
        }

        if (modalDescription) {
            modalDescription.textContent =
                "Atualize as informações jurídicas, processuais e internas do caso.";
        }
    ...
    function configureHeader(url, trigger = null) {
        /*
         * Prioridade:
         *
         * 1. data-case-modal-mode no gatilho, quando informado;
         * 2. detecção pela URL como fallback.
         *
         * Isso evita deixar o modal totalmente dependente
         * do formato da URL.
         */

        const explicitMode =
            trigger?.dataset?.caseModalMode || "";
        ...
        /*
         * Fallback para os gatilhos atuais.
         *
         * A rota de criação é identificada pela presença
         * de "/novo/" na URL.
         */
    ...
    async function loadForm() {
        ...
        try {
            ...
            console.error(
                "Erro ao carregar formulário da causa:",
                error
            );
        ...
        try {
            ...
            if (
                ...
                if (data.success) {
                     ...
                     * CRIAÇÃO
                     *
                     * O backend devolve redirect_url para o
                     * detalhe do caso recém-criado.
                     */
                     ...
                     * EDIÇÃO
                     *
                     * Permanecemos no caso atual e recarregamos
                     * a página para atualizar cards, indicadores
                     * e histórico.
                     */
            ...
            throw new Error(
                `Erro ao salvar causa: ${response.status}`
            );

        } catch (error) {
            console.error(
                "Erro ao salvar causa:",
                error
            );
            ...
            /*
             * Se houve erro de validação, o formulário original
             * pode ter sido substituído pelo HTML devolvido
             * pelo Django.
             */
