(() => {
    ...
    const modalBody = document.getElementById(
        "required-document-modal-body"
    );

    const modalPanel = modal.querySelector(
        "[data-required-document-modal-panel]"
    );

    const backdrop = modal.querySelector(
        "[data-required-document-modal-backdrop]"
    );

    let triggerElement = null;
    let requiredDocumentUrl = "";
    ...
    const loadingTemplate = `
        <div
            data-required-document-modal-loading
    ...
    const errorTemplate = `
        <div
            data-required-document-modal-error
            class="flex min-h-64 items-center justify-center"
        >
            <div class="max-w-sm text-center">
                ...
                <button
                    type="button"
                    data-required-document-modal-retry
    ...
    async function loadForm() {
        ...
        try {
            ...
            console.error(
                "Erro ao carregar formulário de documento:",
                error
            );
    ...
    async function submitForm(form) {
        ...
        const submitLabel = form.querySelector(
            "[data-required-document-submit-label]"
        );
        ...
        try {
            ...
            const response = await fetch(
                form.action || requiredDocumentUrl,
                {
                    method: "POST",
            ...
            throw new Error(
                `Erro ao adicionar documento: ${response.status}`
            );

        } catch (error) {
            console.error(
                "Erro ao adicionar documento necessário:",
                error
            );
            ...
            /*
             * O formulário pode ter sido substituído pelo HTML
             * devolvido pelo backend em caso de validação.
             */
    ...
    document.addEventListener(
        ...
        (event) => {
            ...
            const trigger = event.target.closest(
                "[data-required-document-modal-open]"
            );

            if (trigger) {
                const url =
                    trigger.dataset.requiredDocumentUrl ||
            ...
            const closeButton = event.target.closest(
                "[data-required-document-modal-close]"
            );
